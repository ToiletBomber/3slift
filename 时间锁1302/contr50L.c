...
union{
	...
	MeID.Addr[0]=0;MeID.Addr[1]=ID2;MeID.Addr[2]=ID1;MeID.Addr[3]=ID0;
	AcceptDat.Addr[0]=0;
	BZ=0;
...
//时间校正
	...
	{
		if(Flag_Timer0){i++;Flag_Timer0=0;}
		if(RI)
		{
			RI=0;t=SBUF;
			if(t==TB_EndTime)
			{
				...
				{
					//while(!RI){};RI=0;t=(t>>1)^SBUF;AcceptDat.Addr[2]=t;crc_value=crc_cal_value(t,crc_value);
				...
				if(crc_value==0)
				{
					...
					//修正授权时间
						for(i=0;i<3;i++)
						{
							...
							while(!BZ)
							{
								EraseFlash(i*512);WriteFlash(i*512+0,EndDate.Addr[1]);WriteFlash(i*512+1,EndDate.Addr[2]);WriteFlash(i*512+2,EndDate.Addr[3]);
				...
				for(i=0;i<5;i++){TI=0;SBUF=t;while(!TI){}}
			...
			}else if(t==TB_NowTime)
			{
				...
				while(!BZ)
					...
					EraseFlash(8*512);WriteFlash(8*512,0);//每次校时后，时钟自动修复限次归0
...
//检查时钟是否正常，检查时钟是否停摆，如果停摆报E01，同时显示A01，并禁止运行
	...
	do{t=Read1302 (READ_PROTECT);}while(t&0x7f);//确保1302有0数据输出，0x80或0x00
	...
	if(t&0x80)
	{
		...
		i=ReadFlash(8*512+0);
		if(i>10)BZ=0;//修复时钟限制为10次
		else {EraseFlash(8*512);WriteFlash(8*512,i+1);}
...
//检查是否授权过期，如果过期报E01，同时显示A02，并禁止运行
	EndDate.Addr[0]=0;EndDate.Addr[1]=~EndYear;EndDate.Addr[2]=~EndMon;EndDate.Addr[3]=~EndDay;
	MeID.Addr[0]=0;MeID.Addr[1]=~EndYear1;MeID.Addr[2]=~EndMon1;MeID.Addr[3]=~EndDay1;
	if(EndDate.value!=MeID.value)
	{
		EndDate=MeID;MeID.Addr[0]=0;MeID.Addr[1]=~EndYear2;MeID.Addr[2]=~EndMon2;MeID.Addr[3]=~EndDay2;

		if(EndDate.value!=MeID.value)
		{
			EndDate=MeID;MeID.Addr[0]=0;MeID.Addr[1]=~EndYear;MeID.Addr[2]=~EndMon;MeID.Addr[3]=~EndDay;
...
//存储时钟的年月日时分秒，以备数据丢失可以自行回复使用
	DATE[0]=Read1302(READ_YEAR);DATE[1]=Read1302(READ_MONTH);DATE[2]=Read1302(READ_DATE);DATE[3]=Read1302(READ_HOUR);DATE[4]=Read1302(READ_MINUTE);DATE[5]=Read1302(READ_SECOND);
	...
	for(i=4;i<6;i++)
	{
		...
		while(!BZ)
		{
			EraseFlash(i*512);WriteFlash(i*512+0,DATE[0]);WriteFlash(i*512+1,DATE[1]);WriteFlash(i*512+2,DATE[2]);WriteFlash(i*512+3,DATE[3]);WriteFlash(i*512+4,DATE[4]);WriteFlash(i*512+5,DATE[5]);
...
void Led888(unsigned char Index0,unsigned char Index1,unsigned char Index2)//LED控制
{
	L0=0;L1=0;L2=0;
	if(Line==0)
	{
...
unsigned char t0,t1,t2;
	...
	Led888(Bcd7Seg[t2],Bcd7Seg[t1],Bcd7Seg[t0]);
}
...
unsigned char t0,t1,t2;
	...
	Led888(t2,Bcd7Seg[t1],Bcd7Seg[t0]);
}
...
void ShowBcd()
{
	Led888(Bcd7Seg[LedStr[0]],Bcd7Seg[LedStr[1]],Bcd7Seg[LedStr[2]]);
}

void ShowStr()
{
	Led888(LedStr[0],LedStr[1],LedStr[2]);
}
...
void AdReady(unsigned char INDEX)
{
	...
	//else if(INDEX==3)ADC_CONTR=0xe0;//2.5V
}
...
//读取顺序有电路板决定
	...
	if(!To4){if(State[4]<200)State[4]++;}else{if(State[4]>0)State[4]--;}
	if(!To5){if(State[5]<200)State[5]++;}else{if(State[5]>0)State[5]--;}
	if(!To6){if(State[6]<200)State[6]++;}else{if(State[6]>0)State[6]--;}
...
bit OverLoadCheck()
{
	...
	else if((OverLoad>(OverLoadLimite))|FlagOverLoad)
	{ 	
	...
	*/
	...
	//if(OverLoad<OverLoadLimite1)FlagOverLoad=0;//不过载
	if((OverLoad-(GaoDu/GaoDuK))<OverLoadLimite1)FlagOverLoad=0;//2014/3/19
	else
	{	ErrorState=Alarm+Error+10;//ERROR
		FlagOverLoad=1;
	}
...
*/
	...
	sta=1;FlagRun=0;
/*
	if(State[0]==0)ErrorState=Error+1;//AC24V故障
	if(State[1]==0)ErrorState=Error+1;//钥匙开关
	if(State[2]==0)ErrorState=Error+3;//箱外急停3
	else if(State[3]==0)ErrorState=Error+2;	//箱内急停2
	else if(State[4]==0)ErrorState=Error+1;	//继电器6
	else if(State[5]==0)ErrorState=Error+17;//备用门17
	else if(State[6]==0)ErrorState=Error+16;//门禁16
	else if(State[7]==0)ErrorState=Alarm+Error+4;//紧急上限位4
	else if(State[8]==0)ErrorState=Alarm+Error+18;//安全锁18
	else if(State[9]==0)ErrorState=Alarm+Error+9;//热继9
...
bit sta;
	...
	{
		FlagStart=1;
		if(State[11]<50)ErrorState=Error+11;//上限位动作11
		else if(State[12]<50)ErrorState=Alarm+Error+19;//轮廓上限位动作19
		else if(State[13]<50)ErrorState=Alarm+Error+1;//继电器2错误
		//else {ErrorState=2;FlagRun=1;}//上升状态
	else {ErrorState=2;FlagRun=1;if(GaoDu<60000)GaoDu++;}//2014/3/19
	}else {sta=0;FlagRun=0;}
...
bit sta;
	...
	if(State[14]>150)//下降
	{
		...
		if(State[15]<50){ErrorState=Error+12;GaoDu=0;}//2014/3/19
		else if(State[16]<50)ErrorState=Alarm+Error+1;//继电器1错误
		//else {ErrorState=3;FlagRun=1;}//下降状态
		else {ErrorState=3;FlagRun=1;if(GaoDu>0)GaoDu--;}//2014/3/19
	}else {sta=0;FlagRun=0;}
...
*/
	...
	{
		//DrvRun=FlagOverLoad|FlagErrorPhase|FlagLosePhase;//控制运行继电器
		//DrvRun=FlagOverLoad|FlagErrorPhase|FlagLosePhase|FlagPowerH|FlagPowerL;//控制运行继电器
		//DrvRun=FlagOverLoad|FlagErrorPhase|FlagLosePhase;//控制运行继电器 过压欠压暂不停机，只是报警
		DrvRun=FlagOverLoad|FlagErrorPhase|FlagLosePhase|OverTimer;//2017-5-25
		DrvSl3=0;	//断开安全锁继电器
		//DrvSl3=1;	//test 断开安全锁继电器
	}
	
	//测试端口
	//if(State[17]>150)ErrorState=Alarm+Error+31;//2017-5-27
/*不在测试，张金波20121109
	if((State[16]==0)|(State[17]>0))//测试电路板
	{
		ErrorState=Alarm+Error+1;//电路板故障
	}
*/
}
...
/*
	//电源管理,同时串口波特率加倍
	PCON=0x80;//SMOD/SMOD0/LVDF/POF/GF1/GF0/PD/IDL;
	//定时器工作模式
	TMOD=0x21;//GATE/CT/M1/M0/GATE/CT/M1/M0;
	//串口1控制寄存器
	SCON=0x58;//SM0/SM1/SM2/REN/TB8/RB8/TI/RI
	//中断优先级寄存器低(可位寻址)
	IP=0x02;//PPCA/PLVD/PADC/PS/PT1/PX1/PT0/PX0
	//中断优先级寄存器高(不可位寻址)
	IPH=1;//PPCAH/PLVDH/PADCH/PSH/PT1H/PX1H/PT0H/PX0H
	//定时器控制寄存器
	TCON=0x50//TF1/TR1/TF0/TR0/IE1/IT1/IE0/IT0;TF溢出标志,TR运行控制位,IE中断标志,IT中断源类型选择=0双沿触发,=1下降沿触发
	//TR1=1;TR0=1;
	ET0=1;
*/
	SCON=0x58;
...
/*
			if(ErrorState==2)
			{
				if(WorkTimeStart<10)OneMin++;//升降机上行时间不到10分钟，继续计时

				if(FlagOverTimer){OverTimer=1;OldErrorState=0;}//如果超时并且上升操作,报电路板错误
 			}

			if(OneMin>2400){OneMin=0;WorkTime++;WorkTimeStart++;}//2400*25ms=1分钟
			//if(OneMin>40){OneMin=0;WorkTime++;WorkTimeStart++;}//40*25ms=1秒钟 //test
			if((OldErrorState==2)&(ErrorState!=2))
			{
				if(OldWorkTime!=WorkTime)
				{
					EraseFlash(F_WorkTime);WriteFlash(F_WorkTime,WorkTime/256);WriteFlash(F_WorkTime+1,WorkTime%256);
					EraseFlash(F_WorkTime+512);WriteFlash(F_WorkTime+512,WorkTime/256);WriteFlash(F_WorkTime+513,WorkTime%256);
					EraseFlash(F_WorkTime+1024);WriteFlash(F_WorkTime+1024,WorkTime/256);WriteFlash(F_WorkTime+1025,WorkTime%256);
...
*/
			...
			if(SumCount>(CheckCycle*40))//800us*31*40=992ms
			{
				...
				SumCount=0;
				Sec5++;Sec5=Sec5%5;//2015-08-12
			}

			if(FlagRun)L_Run=(SumCount>(CheckCycle*20));//1赫兹闪烁
			else L_Run=0;
			
			Verify();S2CON&=~S2TI;S2BUF=ErrorState;
...
*/
	...
	if(LoadIndex==1)
	{
		LoadDot[0]=1024-LoadDot[0];//测量的是BA电压，转AB
	 	if(LoadDot[0]>LoadZero)
	 	{
	 		...
	 		//t=LoadDot[0]-LoadZero;t=t*t;SumLoadCurrent[0]=SumLoadCurrent[0]+t;
	 	}else
	 	{
	 		SumLoadCurrent[0]=SumLoadCurrent[0]+LoadZero-LoadDot[0];//2012-11-28
	 		//t=LoadZero-LoadDot[0];t=t*t;SumLoadCurrent[0]=SumLoadCurrent[0]+t;	
	 	}
	...
	if(LoadIndex==2)
	{
	 	if(LoadDot[1]>LoadZero)
	 	{
	 		...
	 		//t=LoadDot[1]-LoadZero;t=t*t;SumLoadCurrent[1]=SumLoadCurrent[1]+t;
	 	}
	 	else 
	 	{
	 		SumLoadCurrent[1]=SumLoadCurrent[1]+LoadZero-LoadDot[1];//2012-11-28
	 		//t=LoadZero-LoadDot[1];t=t*t;SumLoadCurrent[1]=SumLoadCurrent[1]+t;
	 	}
	...
	if(LoadIndex==3)
	{
	 	...
	 	if(LoadDot[2]>LoadZero)
	 	{
	 		...
	 		//t=LoadZero-LoadDot[2];t=t*t;SumLoadCurrent[2]=SumLoadCurrent[2]+t;
	 	}else 
	 	{
	 		SumLoadCurrent[2]=SumLoadCurrent[2]+LoadZero-LoadDot[2];//2012-11-28
	 		//t=LoadDot[2]-LoadZero;t=t*t;SumLoadCurrent[2]=SumLoadCurrent[2]+t;
	 	}
...
//{while(1){TI=0;SBUF=0x01;while(!TI){};Delay(1000);}}
...
*/
	...
	//SCK,IO,RST,NC,NC,NC,NC,NC
	//P6M1=0x00;P6M0=0x05;
	P6M1=0x00;P6M0=0x07;//ds1302数据端口必须上拉才能可靠工作
...
//*********************************************Ok
	...
	FlagPowerL	=0;		//用以检测电压故障 欠压
	FlagPowerH	=0;		//用以检测电压故障 过压
	...
	while(1){RstPower();_nop_();_nop_();_nop_();_nop_();}
	while(1){RstPower();_nop_();_nop_();_nop_();_nop_();}
	while(1){RstPower();_nop_();_nop_();_nop_();_nop_();}
}
...
LedStr[0]=1;i=Read1302 (READ_MONTH);LedStr[1]=i/16;LedStr[2]=i%16;ShowTimer=7;//=7显示bcd7段编码，=8直接显示数据

while(1)
	{
	t=0;
	while(Clock_1s!=0){RstPower();};{Clock_1s++;t++;LedStr[0]=t;i=Read1302 (READ_YEAR);LedStr[1]=i/16;LedStr[2]=i%16;}
	while(Clock_1s!=0){RstPower();};{Clock_1s++;t++;LedStr[0]=t;i=Read1302 (READ_MONTH);LedStr[1]=i/16;LedStr[2]=i%16;}
	while(Clock_1s!=0){RstPower();};{Clock_1s++;t++;LedStr[0]=t;i=Read1302 (READ_DATE);LedStr[1]=i/16;LedStr[2]=i%16;}
	while(Clock_1s!=0){RstPower();};{Clock_1s++;t++;LedStr[0]=t;i=Read1302 (READ_HOUR);LedStr[1]=i/16;LedStr[2]=i%16;}
	while(Clock_1s!=0){RstPower();};{Clock_1s++;t++;LedStr[0]=t;i=Read1302 (READ_MINUTE);LedStr[1]=i/16;LedStr[2]=i%16;}
	while(Clock_1s!=0){RstPower();};{Clock_1s++;t++;LedStr[0]=t;i=Read1302 (READ_SECOND);LedStr[1]=i/16;LedStr[2]=i%16;}
	}
}
