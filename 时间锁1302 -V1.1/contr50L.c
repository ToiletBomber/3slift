...
unsigned int xdata	LoadDot[64]={0};	//任一时刻测量到的电流
...
//检查是否授权过期，如果过期报E01，同时显示A02，并禁止运行
	...
	AcceptDat.Addr[0]=0;
	AcceptDat.Addr[1]=Read1302(READ_YEAR);
	AcceptDat.Addr[2]=Read1302(READ_MONTH);
	AcceptDat.Addr[3]=Read1302(READ_DATE);
...
//	ADC_RESL = 0;	//清结果寄存器
//	CLK_DIV|=0x20;
//	ADC_CONTR = 0x80|0x40|2|0x08;
//	delay();
	
//	CLK_DIV=0x20;        //实际维持复位初值,其中d5/ADRJ位决定ADC结果格式
}
...
*/	
	...
	while(!(ADC_CONTR & 0x10));//等待ADC完成
    ADC_CONTR &= ~0x10;//清除ADC中断
	w = ADC_RES ;
	w = w << 8;
	w |= ADC_RESL;
...
sbit To8	=P7^4;
sbit To9	=P7^5;
sbit To10	=P0^3; //原程序p7^6	  上升检测	 新程序 TO1 p0.3
sbit To11	=P7^7;
sbit To12	=P4^5;
sbit To13	=P4^6;
sbit To14	=P0^2;//原程序 P0^0  下降检测    新程序 TO2 P0.2
sbit To15	=P0^1;
sbit To16	=P0^2;
...
//读取顺序有电路板决定
	...
	if(!To10) //如果电平变低(常态为高)说明上升状态
	{	
		if(A1 != To10); //按下按键to10 = 0 ,a=0
		{	
				FF=0;
				UP = 1;
		}
	...
	if(!To14)//如果电平变低(常态为高)说明下降状态
	{  
		if(B1 != To14); //按下按键to10 = 0 ,a=0
		{
			B1 = To14;
			FF = 0;
			UP = 1;
...
//	for(xx=0;xx<64;xx++)
//	{
//		OverLoad1=LoadDot[xx]+OverLoad1;	
//	}
...
//		WHILE(!TI);
 ...
 /*	if(LoadIndex==0)//0.8ms一次	200us*4
	{
